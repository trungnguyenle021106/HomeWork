...
namespace HomeWork.Way1
{
        public class People
     {
         ...
         {
             int deepth = 0;
             var myAncestors = new HashSet<string>();
             People? current = this;
             // Lưu lại toàn bộ gốc gác của mình (bao gồm cả bản thân)
             while (current != null)
             {
                 myAncestors.Add(current.Id);
                 current = current.Parent;
             }
             ...
             while (current != null && deepth != MAX_SEARCH_DEPTH)
             {
                 if (myAncestors.Contains(current.Id)) return current;
                 current = current.Parent;
                 deepth++;
             }
             return null;
         ...
         public int GetGenerationRelativeTo(People ancestor)
         {
             ...
             while (current != null)
             {
                 if (current.Id == ancestor.Id) return distance;
                 distance++;
                 current = current.Parent;
             }
